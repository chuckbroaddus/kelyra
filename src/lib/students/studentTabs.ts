...
export type StudentViewTab = {
  key: string;
  label: string;
  icon: 'focus' | 'history' | 'work' | 'practice' | 'parents' | 'details';
};
...
/** Office/superintendent seat: Details then Parents only (Chuck order). */
 ...
 * Office (superintendent / administrator) → details+parents.
