...
/** Office/superintendent seat: Classes · Parents · Details (read-only class rows). */
 ...
 * Office (superintendent / administrator) → classes+parents+details.
...
/**
 * Default selected pane for the seat — not derived from tab row index 0.
 * Office keeps Details even when Classes is leftmost; teacher keeps Focus.
 */
export function studentTabDefaultKey(role: string | null | undefined): string {
  return isOfficeChromeRole(role) ? 'details' : 'focus';
}
...
export function studentTabFromParam(
  ...
  const defaultKey = studentTabDefaultKey(role);
