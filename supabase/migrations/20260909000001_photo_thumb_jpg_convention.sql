-- t_903cdfa6: align photo_thumb_path with backfill + client (`_thumb.jpg` always).
-- Idempotent. Also expose legacy source-ext helper for older objects.
...
create or replace function public.photo_thumb_path_legacy(p_path text)
returns text
language sql
immutable
parallel safe
...
grant execute on function public.photo_thumb_path_legacy(text) to authenticated, anon;
  ...
  select exists (
    ...
    where me.id = auth.uid()
      ...
      and (
        ...
        or public.photo_thumb_path_legacy(a.storage_path) = p_path
      )
  );
$$;
    ...
    select exists (
      ...
      where m.profile_id = auth.uid()
        and (
          ...
          or public.photo_thumb_path_legacy(t.photo_path) = $1
        )
    )
  $sql$
  into ok
  ...
  select exists (
    ...
    where mem.profile_id = auth.uid()
      ...
      and (
        ...
        or public.photo_thumb_path_legacy(m.payload->>'storage_path') = p_path
      )
  );
$$;
  ...
  select exists (
    ...
    where p.payload is not null
      and (
        ...
        or public.photo_thumb_path_legacy(p.payload->>'storage_path') = p_path
      )
  ...
  or exists (
    ...
    where r.payload is not null
      and (
        ...
        or public.photo_thumb_path_legacy(r.payload->>'storage_path') = p_path
      )
      and public.can_see_post(p)
  );
$$;
...
begin
  ...
  if kind = 'photo' then
    ...
    delete from storage.objects
    where bucket_id = bucket
      and name = public.photo_thumb_path_legacy(path)
      and name is distinct from public.photo_thumb_path(path);
  end if;
