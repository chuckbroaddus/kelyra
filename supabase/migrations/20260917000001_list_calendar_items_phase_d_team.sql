-- CAL-R2 Phase C: list_calendar_items absence audience + parent personal.
-- Owner parents of focused child C see C's absences (not creator-only).
-- Parent personal (visibility_scope=self) listed. Student still never sees absence.
-- UNION ORDER BY aliases preserved (t_dafdba90). Never teaches_class / is_staff.
-- New file only — do not rewrite applied 20260912000001 / 20260913000002.
        ...
        and (
          ...
          or (
            ...
            and (
              ...
              or (
                e.visibility_scope = 'team'
                and e.team_id is not null
                and public.i_calendar_team_member(e.team_id, null)
              )
            )
          )
        )
      ...
      where e.deleted_at is null
        ...
        and (
          ...
          or (
            e.visibility_scope = 'team'
            and e.team_id is not null
            and public.i_calendar_team_member(e.team_id, public.my_student_id())
          )
        )
        and (
          p_calendar_ids is null
    ...
    (
      ...
      where e.deleted_at is null
        ...
        and (
          ...
          -- Parent absence: owner parents of focused child C (not creator-only).
          -- Never other twin (student_id = child). Never school-wide / office.
          -- Student v1 does not see own absence (student branch omits student_teachers).
          or (
            e.visibility_scope = 'student_teachers'
            and e.category = 'absence'
            and e.student_id = child
          )
          -- Parent personal (self only)
          or (
            e.visibility_scope = 'self'
            and e.owner_profile_id = me
          )
          or (
            e.visibility_scope = 'team'
            and e.team_id is not null
            and public.i_calendar_team_member(e.team_id, child)
          )
        )
        and (
          p_calendar_ids is null
