-- List/avatar derivatives live next to originals in the private photos bucket.
-- `{name}.jpg` → `{name}_thumb.jpg`. Lists sign the thumb; review/AI keep the original.
...
declare
  ok boolean := false;
begin
  -- Live message_threads has no photo_path (20260819000001 never applied).
  -- LANGUAGE sql would fail CREATE with 42703. Probe the catalog, then
  -- dynamic SQL so a later photo_path column still matches thumbs.
