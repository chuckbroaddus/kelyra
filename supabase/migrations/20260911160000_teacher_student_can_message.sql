-- Allow teacher ↔ own-student messaging (1:1 and small groups).
-- Live can_message still had from_staff→to_student return false (v1).
-- open_group_thread still used all_staff / others_parents and blocked student groups.
-- Co-teachers: class_teachers seat, not only classes.teacher_id.
-- Chuck 2026-09-11: Students Message select mode must work for roster kids with logins.
...
begin
      ...
      where s.id = to_row.student_id
        and (
          ...
          or exists (
            ...
            where e.student_id = s.id and ct.teacher_id = p_from
          )
        )
    );
  end if;
      ...
      where ps.parent_id = from_row.parent_id
        and (
          ...
          or exists (
            ...
            from public.enrollments e
            join public.class_teachers ct on ct.class_id = e.class_id
            where e.student_id = s.id and ct.teacher_id = p_to
          )
        )
    );
  end if;
  ...
  if from_student and to_staff then
    ...
    return exists (
      ...
      from public.enrollments e
      join public.class_teachers ct on ct.class_id = e.class_id
      where e.student_id = from_row.student_id and ct.teacher_id = p_to
    );
  end if;
