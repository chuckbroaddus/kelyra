...
| Teacher | 1:1 | 1:1 and staff group | 1:1 or small group (one child or selected parents; max 11) | **Family thread**: single student → durable per-child thread (`student_id`); multi-select → **one shared** `family_lock` group (teacher + selected students + their linked parent logins; students + parents locked). Pick cap 11; group member cap 12. Class-wide stays a **post** |
...
| Selected students | Class / Students — **LinkedParents** under each student (always, mirror Parents **LinkedKids**). Messaging select: **Message these students** → checkboxes / Select all + Cancel → **Message N student(s)** (mute rules + grayed-out help sit below that CTA in the footer). **1 eligible** → `open_student_family_thread` (durable per-child `student_id` thread). **≥2 eligible** → **one shared** `open_multi_student_family_thread` group (`family_lock=true`, `student_id=null`): teacher + all selected student logins + all linked parent logins for those students. Chuck 2026-09-11: shared multi-family over separate threads (parents in the group see other selected students). Selectable only when student has login **and** ≥1 linked parent **and** at least one linked parent has a login. Skip/gate others (**Needs login** / **Needs parents** / **Needs parent login**). Navigate to that one thread; CTA e.g. **Opened 1 shared group with 3 students — parents included**. Pick cap 11; SQL rejects if total members > 12. Students + parents locked on family threads (SQL + UI). Migrations CoS apply. |
...
3b. Class / Students: each student row shows linked **parents underneath** (avatars + first names; **Needs login** on parent chips without profiles) in normal list and select mode. Tap **Message these students**, select three fully-covered students, **Message 3 students**. Cancel clears selection. Select UX may still cap at 11 picks. Selectable only when: student login + ≥1 linked parent + at least one linked parent has a login. Grayed rows show **Needs login** / **Needs parents** / **Needs parent login**. On send: 1 eligible → `open_student_family_thread`; ≥2 eligible → one `open_multi_student_family_thread` shared group; navigate to that thread; CTA e.g. **Opened 1 shared group with 3 students — parents included** (skips / over-cap noted). Class-wide remains **Post to class**. Parents + students are locked on family threads (no Remove). Parents in a multi-select group see the other selected students.
...
- RPC `open_student_family_thread(p_student_id)` — staff who teach the student; members = teacher + student profile + linked parent profiles that exist.
- Hard-fail if no parents linked, zero parent logins, or student lacks a login. At least one linked parent with a login is enough (include all parents who have logins; skip parents without). Never open teacher+student-only.
- Reuse `message_threads.student_id` for one durable thread per student; ensure members on reopen; never drop parents.
- **Multi-select (Chuck 2026-09-11):** RPC `open_multi_student_family_thread(p_student_ids uuid[])` — staff only; each id must be taught by me; dedupe; require ≥2 (else client uses single RPC). Hard-fail if any selected student fails the gate (same login/parent rules). Members = me ∪ all student profiles ∪ all parent profiles with logins. Dual-hat OK. Cap 12 total members. Insert `kind=group`, `student_id=null`, `family_lock=true`, title = joined first names or `Students`. v1 always creates a new thread. Migration `20260911174000_open_multi_student_family_thread.sql` — **CoS apply** (authorized); do not merge without apply note.
- FERPA: Chuck explicitly chose **one shared multi-family group** over separate per-student threads for multi-select. Parents in that group see the other selected students.
- `remove_group_member`: nobody may remove student or parent profiles when `family_lock` OR `student_id` is set (same message as today). Mute allowed. `add_group_member`: staff only on family threads (`student_id` or `family_lock`).
...
- One-parent-login gate: `20260911172000_open_student_family_thread_one_parent_login.sql` — CoS applied live 2026-09-11 (matches client `familyGate`).
