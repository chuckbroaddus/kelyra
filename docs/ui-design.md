...
**Teacher.** Section **Students enrolled** (uppercase section title) → vertical `ListRow`s (photo + name; tap → student page). Bottom rounded primary CTA **Message these students** (same `PrimaryButton` / `radius.md` pattern as Parents **Message these parents** / Assignments **Create Assignment**) enters **messaging select mode**: checkbox column on each row; **Select all** checkbox at the top of that column with **Cancel** on the **same row**, far right (Select all left / Cancel trailing) — Cancel exits (clears selection, restores normal list). While select mode is idle (none checked), footer is disabled **Message these students**. When any student is checked, **Message N student(s)** takes that primary slot (resolve `profiles` via `student_id`; max 11; teacher→own-student DMs/groups are allowed — class-wide stays **Post**). Entering select mode batch-looks-up login profiles (non-blocking); students without a linked profile show **Needs login** and are not checkable once known. **Select all** may check selectable students; if more than 11 are checked, the Message CTA is **disabled** with adjacent mute helper **Group chats stay small. Pick at most 11 students.** (visible next to the button — not a buried bottom error). **Zero** of the selected have logins → Message disabled + CTA-adjacent **Those students need logins first** (also surface profiles query errors beside the CTA — never buried-only). **Mixed** pick → open the group with those who have logins, brief CTA note like **Messaging 4 of 7 — 3 need logins**, navigate into the thread. Teachers get messaging select mode even though the list is browse-only (messaging ≠ add/remove). No **Add students** lead/banner. No helper about office / All students. No horizontal `AvatarTray`. No swipe **Remove**. No **All students** enroll list / swipe **Add**. Empty: `No students enrolled yet. The office manages the class roster.`

**Office/admin.** Add students card (photo / record / type / confirm checklist) + parked roster-import card if any (§20) → **Students enrolled** vertical `ListRow`s with swipe **Remove** (hidden while messaging select mode) + same **Message these students** select mode as teachers → **All students** with swipe **Add** to enroll someone already at the school → last ghost **Delete class** (type-the-name; office/admin + `classes.delete` only — teachers never delete classes). Same office surfaces also live on `/admin/class/{id}` (Students pane). Feed icon and Syllabus editing live on **Settings** (`/class/{id}/settings`), not on this roster screen.
