...
**Teacher.** Section **Students enrolled** (uppercase section title) → vertical `ListRow`s (photo + name + **LinkedParents** under each student — avatars + first names, mirror Parents **LinkedKids**; parent chips without a login show **Needs login**). Tap → student page. Bottom rounded primary CTA **Message these students** enters **messaging select mode**: checkboxes; **Select all** + **Cancel** same row (Cancel far right) — that row stays clean (controls only). Mute help under the **Message N** footer CTA: parents always included / cannot be removed; one student opens that family's chat, two or more open one shared group; plus grayed-out gate copy when any row is blocked. **1 eligible** → that student’s durable family chat; **≥2 eligible** → **one shared** multi-student family group (teacher + selected students + their linked parent logins; parents see other selected students). Selectable only when student has a login, ≥1 linked parent, and at least one linked parent has a login — otherwise dim with **Needs login** / **Needs parents** / **Needs parent login**. Select UX may still cap at 11 picks (SQL caps shared group at 12 people total). Send navigates to that one thread; CTA e.g. **Opened 1 shared group with 3 students — parents included** (skips / over-cap noted). Teachers get messaging select mode even though the list is browse-only. No **Add students** lead/banner. No horizontal `AvatarTray`. No swipe **Remove**. No **All students** enroll list. Empty: `No students enrolled yet. The office manages the class roster.`

**Office/admin.** Add students card (photo / record / type / confirm checklist) + parked roster-import card if any (§20) → **Students enrolled** vertical `ListRow`s with **LinkedParents** under each student + swipe **Remove** (hidden while messaging select mode) + same **Message these students** family-thread select mode as teachers → **All students** with swipe **Add** to enroll someone already at the school → last ghost **Delete class** (type-the-name; office/admin + `classes.delete` only — teachers never delete classes). Same office surfaces also live on `/admin/class/{id}` (Students pane). Feed icon and Syllabus editing live on **Settings** (`/class/{id}/settings`), not on this roster screen.
...
**Teacher.** Section **Parents of class' students** → vertical `ListRow`s (photo + name + linked kids; tap → parent page). Bottom rounded primary CTA **Message these parents** (same `PrimaryButton` / `radius.md` pattern as Assignments **Create Assignment**) enters **messaging select mode**: checkbox column on each row; **Select all** checkbox at the top of that column with **Cancel** on the **same row**, far right (Select all left / Cancel trailing) — Cancel exits (clears selection, restores normal list). While select mode is idle (none checked), footer is disabled **Message these parents**. When any parent is checked, **Message N parent(s)** takes that primary slot (resolve `profiles` via `parent_id`; max 11). Same login UX as Students: batch-lookup on enter; **Needs login** rows not checkable once known; **zero** logins among selected → Message disabled + CTA-adjacent hard-fail; **mixed** → message those with logins, note e.g. **Messaging 4 of 7 — 3 need logins**, proceed into the thread; surface profiles query errors beside the CTA. **Select all** may check selectable parents; if more than 11 are checked, the Message CTA is **disabled** with adjacent mute helper **Group chats stay small. Pick at most 11 parents.** (visible next to the button — not a buried bottom error). Teachers get messaging select mode even though the list is browse-only (messaging ≠ add/remove). No **Add parent** field/button. No photograph-a-contact-card affordance on this tab. No swipe **Remove**. No **All parents** section / swipe **Add**. No PhaseBanner / no **PHASE 4 · Family** lead. Empty: `No parents linked to students in this class yet. The office manages the class family list.`
...
**Selected-name morph.** Switching tabs animates only the old↔new pills (intermediates stay icon-only) with `chrome.motion.personTab` (325 ms = `context` × 1.25). Icon stays **left-pinned** in both states; the pill’s right edge grows/shrinks so the label reveals LTR (first letter first) and hides under that right edge — never clip the glyph. Soft-fill (`brandSoft`) fades with the expand value. The outgoing label stays mounted and clips away — do not unmount it at the start of deselect. Marquee only after the selected pill reaches max width. Reduce Motion jumps (`setValue` / duration 0) and `scrollTo(..., animated: false)`.
