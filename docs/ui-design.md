...
**Teacher.** Section **Students enrolled** (uppercase section title) → vertical `ListRow`s (photo + name + **LinkedParents** under each student — avatars + first names, mirror Parents **LinkedKids**; parent chips without a login show **Needs login**). Tap → student page. Bottom rounded primary CTA **Message these students** enters **messaging select mode**: checkboxes; **Select all** + **Cancel** same row (Cancel far right). Parents are always included and cannot be removed; each selected student opens their **own family chat** (not a shared multi-student group). Selectable only when student has a login, ≥1 linked parent, and every linked parent has a login — otherwise dim with **Needs login** / **Needs parents** / **Needs parent login**. Select UX may still cap at 11 picks. Send opens N `open_student_family_thread` calls, navigates to the first, CTA e.g. **Opened 3 chats — parents included**. Teachers get messaging select mode even though the list is browse-only. No **Add students** lead/banner. No horizontal `AvatarTray`. No swipe **Remove**. No **All students** enroll list. Empty: `No students enrolled yet. The office manages the class roster.`
